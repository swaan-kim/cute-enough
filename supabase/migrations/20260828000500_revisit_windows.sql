-- Keep a revealed photo reopenable until the next recharge boundary instead of KST midnight.
alter table public.daily_reveals
  add column if not exists revisit_until timestamptz;

update public.daily_reveals
set revisit_until = created_at + interval '3 hours'
where revisit_until is null;

alter table public.daily_reveals
  alter column revisit_until set default (now() + interval '3 hours'),
  alter column revisit_until set not null;

-- A pet can be met again with a newly recharged ticket after its revisit window expires.
drop index if exists public.reveals_one_pet_per_day_idx;

create index if not exists reveals_active_revisit_idx
  on public.daily_reveals(owner_hash, revisit_until desc, pet_id);
...
begin
  -- This lock spans KST midnight so the same viewer cannot be charged twice for
  -- one still-active pet through concurrent requests.
  ...
  if p_unlock_method = 'UPLOAD' then
    ...
    v_revisit_until := v_now + interval '3 hours';
    ...
    v_revisit_until := refill_anchor + interval '3 hours';
