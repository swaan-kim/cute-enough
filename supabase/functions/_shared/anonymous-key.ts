...
type AnonymousKeyVerificationPayload = {
  resultType?: string;
  success?: boolean | string;
  error?: { errorCode?: string };
};

export function parseAnonymousKeyVerificationSuccess(payload: unknown): boolean {
  if (typeof payload !== 'object' || payload === null) return false;

  const candidate = payload as AnonymousKeyVerificationPayload;
  return candidate.resultType === 'SUCCESS'
    && (candidate.success === true || candidate.success === 'true');
}
...
export async function verifyAnonymousKey(anonymousKey: string): Promise<void> {
  ...
  try {
    ...
    console.warn('anonymous key verification unavailable', {
      upstreamStatus: null,
      resultType: null,
      errorCode: null,
    });
