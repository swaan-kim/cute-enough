...
Scenarios cover two free visits followed by an earned ad visit, cancellation, failed native load, duplicate earned events, manual return, and collect/owner/replay requests on the second input. The first input requests nothing. Leaving after the second preserves any committed collection and ticket use; a late response never reopens the modal, and revisiting does not charge again. Slow downloads keep one skeleton until decoding completes. The third input still ends with a 900 ms smile/tail reaction (first two: 450 ms); reduced-motion keeps the smile without jump/wag.

Browser-only timing records input, API request/response, image download/decode, modal insertion and photo visibility. Timing entries contain no identity or URL; the separate API evidence uses dummy local fixture identities only. One 390 px comparison changes only the served PlayScene request trigger to reproduce the former third-input timing, with the same controlled image and interaction delays. This is a local 1 px PNG experiment, not physical-phone or production-server performance evidence. Playwright retains traces/screenshots and attaches the timing results; no timing telemetry ships in production.
...
The current actual-Edge suite covers `photoPrepare`, final owner-photo signing failure/retry, earned ads, and rate-limit rejection without extra allowance reads. It does not claim actual entry-point coverage for upload/share/favorite dispatch or the notification worker. Passing fake transport and PGlite checks is not evidence of a deployed database, a running schedule, push delivery, SDK availability or real ad inventory.

The current client does not call `photoPrepare`: encounter requests start on the second input, while the modal waits for the third input and its 900 ms reaction. Post-record album and reward reads overlap; authorization, signing and atomic recording stay ordered. The retained preparation endpoint is not a client dependency and is excluded from this Edge release. Only if separately deploying that older endpoint, apply and verify the additive `supabase/migrations/20260912000100_photo_prepare_rate_limit.sql` migration. The prior SQL action constraint rejects `photoPrepare`. SQL application, Edge deployment, AIT build/upload and individual dog publication are distinct steps. Exact operational gates and current QA results belong in `docs/quality/QA_STRATEGY.md`.
