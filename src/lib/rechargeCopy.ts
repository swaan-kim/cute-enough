...
/**
 * 다음 무료 티켓 충전 시각을 홈 카드에 보여줄 짧은 문구로 만든다.
...
export function formatRechargeCountdown(nextChargeAt: string | undefined, now = new Date()): string | undefined {
  ...
  if (remainingMs < MINUTE_MS) return '곧 1장 충전';

  const totalMinutes = Math.ceil(remainingMs / MINUTE_MS);
  const hours = Math.floor(totalMinutes / 60);
  const minutes = totalMinutes % 60;
  if (hours <= 0) return `${minutes}분 뒤 1장 충전`;
  if (minutes === 0) return `${hours}시간 뒤 1장 충전`;
  return `${hours}시간 ${minutes}분 뒤 1장 충전`;
}
