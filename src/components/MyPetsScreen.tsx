...
export function MyPetsScreen({ pets, loading, error, uploadRewardPetId, onRetry, onUpload, onMeet, onShare, onAddPhotos }: {
  ...
  return (
    <main className="my-pets-screen">
      ...
      />
      {loading && pets.length === 0 ? <p className="my-pets-message" role="status">강아지들을 불러오는 중…</p>
        : error && pets.length === 0 ? <div className="my-pets-message" role="status"><p>{error}</p><Button size="medium" color="dark" variant="weak" onClick={onRetry}>다시 불러오기</Button></div>
          : pets.length === 0 ? <div className="my-pets-message"><strong>아직 소개한 강아지가 없어요</strong><p>사진 한 장으로 귀여운 캐릭터를 만들어보세요.</p><Button size="medium" onClick={onUpload}>강아지 소개하기</Button></div>
            : <>
              {(loading || error) && <div className="album-refresh-error" role="status">
                <p>{loading ? '등록 상태를 새로 확인하고 있어요…' : error}</p>
                {!loading && error && <Button size="small" color="dark" variant="weak" onClick={onRetry}>다시 불러오기</Button>}
              </div>}
