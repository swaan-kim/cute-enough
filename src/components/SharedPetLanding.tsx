...
export function SharedPetLanding({ pet, onMeet, onHome }: { pet: PetSummary; onMeet: () => void; onHome: () => void }) {
  ...
  const metToday = Boolean(pet.revealedToday);
  return (
    <main className="shared-screen">
      <Top
        ...
        subtitleBottom={<Top.SubtitleParagraph>{pending
          ...
          : metToday ? '오늘 만난 친구예요. 사진을 바로 다시 볼 수 있어요.' : '간식을 주고 귀여운 모습을 만나보세요.'}</Top.SubtitleParagraph>}
      />
      ...
      <div className="shared-actions">
        <Button display="full" size="large" onClick={onMeet}>{pending ? '캐릭터와 놀아보기' : metToday ? '사진 다시 보기' : '이 친구 만나기'}</Button>
