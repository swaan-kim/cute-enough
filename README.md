...
작고 귀여운 상호작용 하나로 잠깐 웃을 수 있는 경험을 목표로 합니다. 공개 강아지 5마리는 사용자별로 KST 하루 동안 고정되며, 자정이 지나면 새 조합으로 바뀝니다. 무료 이용권은 최대 2개까지 보관되고 사용 후 3시간마다 1개씩 충전됩니다. 이용권이 0개일 때는 광고 승인이 완료된 운영 환경에서 리워드 광고로 기다림을 건너뛸 수 있습니다(하루 최대 2회). 본인이 가장 최근에 올린 `pending | approved` 강아지는 오늘의 진행도와 별개인 여섯 번째 보너스 친구로 표시되며 언제든 사진을 볼 수 있습니다.
...
- 오늘의 집: 사용자·KST 날짜별 공개견 5개 슬롯을 저장해 같은 날에는 순서와 구성을 유지하고 자정에만 새로 선정
- 내 강아지 보너스: 가장 최근의 `pending | approved` 강아지를 공개 5마리와 중복되지 않는 소파 전용 여섯 번째 친구로 표시하며 이용권·광고·`5/5` 진행도에서 제외
...
| URL | 확인할 상태 |
| --- | --- |
| `/?scenario=five` | 공개 샘플 강아지 5마리와 최초 공개·재열람 |
| `/?scenario=owner` | 공개 5마리 + 소파의 `내 샘플` 보너스 친구 |
| `/?scenario=complete` | 오늘의 친구 5/5 완료 + 다섯 마리 사진 다시 보기 |
...
업로드한 강아지는 검수 전에도 업로더 본인의 집과 등록 완료 화면에 바로 나타납니다. 가장 최근의 `pending | approved` 강아지는 소파의 `내 강아지` 보너스 자리에 표시되고, 이용권·광고·오늘의 `5/5` 진행도를 사용하지 않은 채 사진을 바로 볼 수 있습니다. 검수 중 공유 링크를 받은 타인에게는 캐릭터만 보이고 원본 사진은 노출되지 않습니다. 승인 즉시 같은 공유 링크와 업로더 화면에 최종 디자인이 반영되고, 다른 사용자의 집 후보에는 다음 KST 날짜부터 편입됩니다.

한 번 연 실제 사진은 해금 방식과 무관하게 다음 무료 이용권 충전 경계까지 바로 다시 볼 수 있습니다. 이용권이 이미 두 장으로 가득 차 충전 시각이 없는 경우에만 연 시점부터 3시간을 사용합니다.

홈은 `daily_house_assignments`에 저장된 공개 강아지 5개 슬롯과 별도의 `ownerBonusPet`을 보여줍니다. 공개 슬롯은 사용자·KST 날짜별로 고정되고 공개 중지된 강아지의 슬롯만 교체됩니다. 승인견 후보가 부족해도 같은 강아지를 복제하지 않으며 빈 슬롯을 안내 상태로 남깁니다. 전체 업로드 이력은 `내가 소개한 강아지`에서 최신순으로 확인할 수 있습니다.
...
| 필드 | 의미 |
| --- | --- |
| `dailyPets` | 오늘 고정된 공개 강아지 최대 5마리와 `houseSlot` |
| `ownerBonusPet` | 본인의 최신 `pending | approved` 강아지 또는 `null` |
| `dailyProgress` | KST 날짜, 만난 공개견 ID, `metCount`, 고정 `totalCount: 5`, 완료 여부 |
...
웹 데모에는 실제 강아지 하늘(2장)·구르미(3장)의 메타데이터 제거 JPG가 들어 있습니다. 사진마다 다른 이름과 ID를 붙이지 않고 실제 한 마리당 하나의 `pet.id`로 관리합니다. private/production 빌드에서는 이 정적 파일을 번들에서 제거하며, 운영에서는 같은 사진을 private Storage에 먼저 등록해야 합니다. 이 seed는 연결 확인용이므로 v2의 공개 5슬롯을 모두 채우지 못합니다. 정상 운영 전에는 별도로 활성 승인견을 최소 8마리 확보합니다.
