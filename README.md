...
작고 귀여운 상호작용 하나로 잠깐 웃을 수 있는 경험을 목표로 합니다. 무료 만남 이용권은 최대 2개까지 보관되며, 사용한 뒤 3시간마다 1개씩 다시 충전됩니다. 집에는 사용자별로 고정된 오늘의 강아지 최대 5마리가 나타나고 KST 자정에 새로운 조합으로 바뀝니다. 이용권을 모두 쓴 뒤에는 광고 승인이 완료된 운영 환경에서 리워드 광고 1회당 한 친구를 추가로 만날 수 있습니다(하루 최대 2회). 반려견 사진을 등록한 날에는 본인이 올린 강아지와 한 번 무료로 놀 수 있습니다.
...
- 이용 규칙: 기본 이용권 최대 2개, 사용 후 3시간마다 1개 충전, 이후 리워드 광고 1회당 추가 1회(하루 광고 최대 2회), 그날 첫 사진 등록 강아지 전용 보너스 1회
- 오늘의 집: 사용자·KST 날짜 기준으로 최대 5마리를 결정해 같은 날에는 순서와 구성이 유지되고 자정에만 교체
- 다시 보기: 당일 이미 만난 강아지는 이용권을 추가로 쓰지 않고 실제 사진을 바로 다시 열기
- 반려견 등록: 사진 선택, 특징 확인, 동의 및 검수 요청 후 가장 최근에 올린 강아지가 본인 집에 즉시 등장
- 친구 공유: 승인된 강아지는 실제 사진까지, 검수 중 강아지는 캐릭터만 보여주는 전용 딥링크
- 사진 저장: 실제 사진에 작은 `찰딱` 워터마크를 합성한 JPG 복사본만 사용자 기기에 저장
...
업로드한 강아지는 검수 전에도 업로더 본인의 집에 나타나며, 그 강아지에 묶인 당일 보너스로 한 번 만날 수 있습니다. 보너스를 사용한 뒤에도 캐릭터는 집에 남습니다. 검수 중 공유 링크를 받은 사람에게는 캐릭터만 보이고 원본 사진은 노출되지 않습니다. 승인된 뒤에는 다른 사용자의 집 후보와 공유 링크에서 실제 사진을 만날 수 있습니다. `20260824000200_upload_rewards_and_sharing.sql` 이전의 기존 업로드에는 보상이 소급 적용되지 않습니다.

홈은 사용자별로 KST 날짜에 고정된 강아지를 한 번에 최대 다섯 마리 보여줍니다. 업로더에게는 가장 최근의 본인 강아지 한 마리를 우선 배치하고, 나머지는 승인된 공개 강아지에서 결정적으로 섞어 채웁니다. 전체 업로드 이력은 `내가 올린 강아지`에서 최신순으로 계속 확인할 수 있습니다.

### 초기 강아지 2마리·실사 5장 등록

웹 데모에는 실제 강아지 하늘(2장)·구르미(3장)의 메타데이터 제거 JPG가 들어 있습니다. 사진마다 다른 이름과 ID를 붙이지 않고 실제 한 마리당 하나의 `pet.id`로 관리합니다. private/production 빌드에서는 이 정적 파일을 번들에서 제거하며, 운영에서는 같은 사진을 private Storage에 먼저 등록해야 합니다.
...
사용자 사진 승인은 Dashboard에서 상태만 직접 바꾸지 말고 `review_pet_submission(pet_id, 'approved' 또는 'rejected', 검수자, 사유)` RPC를 사용합니다. 이 함수가 상태·검수 시각·감사 로그를 한 트랜잭션으로 기록합니다.
