...
업로드한 강아지는 검수 전에도 업로더 본인의 집과 등록 완료 화면에 바로 나타납니다. 첫 등록 보너스로는 간식 상호작용 후 사진을 보고, 그 뒤에는 `pending`과 `approved` 상태에서 이용권 차감 없이 본인 사진을 바로 다시 볼 수 있습니다. 검수 중 공유 링크를 받은 타인에게는 캐릭터만 보이고 원본 사진은 노출되지 않습니다. 승인 즉시 공유 링크가 갱신되고, 다른 사용자의 집 후보에는 다음 KST 날짜부터 편입됩니다.

한 번 연 실제 사진은 해금 방식과 무관하게 다음 무료 이용권 충전 경계까지 바로 다시 볼 수 있습니다. 이용권이 이미 두 장으로 가득 차 충전 시각이 없는 경우에만 연 시점부터 3시간을 사용합니다.

홈은 사용자별로 KST 날짜에 고정된 강아지를 한 번에 최대 다섯 마리 보여줍니다. 업로더에게는 가장 최근의 본인 강아지 한 마리를 우선 배치하고, 나머지는 승인된 공개 강아지에서 결정적으로 섞어 채웁니다. 전체 업로드 이력은 `내가 소개한 강아지`에서 최신순으로 확인할 수 있습니다.
