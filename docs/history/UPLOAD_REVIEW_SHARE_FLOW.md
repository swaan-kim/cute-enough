...
현재 운영 범위에는 공개 미니앱 안의 관리자 화면을 두지 않는다. 숨겨진 공개 주소에 서비스 역할 키를 넣는 방식은 권한 보호가 되지 않으므로, 초기 운영은 `127.0.0.1`에만 열리는 내부 검수 페이지에서 진행한다. 설정 방법은 [내부 강아지 검수 페이지](../operations/REVIEW_CONSOLE.md)를 따른다.
